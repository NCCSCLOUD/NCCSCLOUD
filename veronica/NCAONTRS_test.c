...
int NCAONTRS_test()
{
	...
	if(content==NULL)
	{
		show_error("NCAONTRS","NCAONTRS_test","No memory");
		return FALSE;
	}
	...
	k=4;
	fout=fopen(".\\NCAONT-RSVM1.txt","w");
	if(fout==NULL)
	{
		show_error("NCAONTRS","NCAONTRS_test","output file cannot open");
		return FALSE;
	}
	for(r=0;r<10;r++)//10轮
	{
		for(i=0;i<7;i++)
		{
			...
			for(n=4;n<7;n++)
			{
				...
				codeptr->codenum=0;
				codeptr->w=8;
				NCAONTRS_test_set_address(addptr,n);
				//while(!feof(fin))太多了
				randnum=rand()%1000;
				...
				for(itemindex=0;itemindex<100;itemindex++)
				{
					...
					if(resTF==FALSE)
					{
						show_error("NCAONTRS","NCAONTRS_test","the traceitem cannot be read");
						return FALSE;
					}
					strlen=itemptr->length;
					NCAONTRS_DEVICE_API(content,strlen,addlistptr);
					endtick=clock();
