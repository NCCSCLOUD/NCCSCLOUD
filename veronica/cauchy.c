...
int cauchy_API_Packetgroup(AONTSHAREGROUP *tarpacketgtr,AONTSHAREGROUP *soupacketgtr,CODEPARA *codeptr)
{
	BITMATRIX *matrix;
	...
	int resTF;
	int i=0,packetsize;
	int totnum,datanum,codenum,pronum,w,rows,cols;
	int *conptr;
	//参数考出来
	totnum=codeptr->totnum;
	datanum=codeptr->datanum;
	...
	if(matrix==NULL)
	{
		show_error("cauchy","cauchy_API_Packetgroup","No memory");
		return FALSE;
	}
	matrix->w=w;
	rows=codenum*w;
	...
	//matrix->matrix=mat;
	//进行编码操作
	resTF=matrix_matrix_calculate_packet(matrix,codeptr,tarpacketgtr,soupacketgtr);
	if(resTF==FALSE)
	{
		show_error("cauchy","cauchy_API_Packetgroup","the results can calculate");
		return FALSE;
	}
	free(mat);
	free(matrix);
