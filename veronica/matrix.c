...
//根据bit阵计算packet
int matrix_matrix_calculate_packet(BITMATRIX *matrixptr,CODEPARA *codeptr,AONTSHAREGROUP *tarpacketgtr,AONTSHAREGROUP *soupacketgtr)
{
	int i,j,k,windex,index,resTF;
	int totnum,datanum,codenum,pronum,w;
	int tarpacketnum,soupacketnum,packetsize;
	AONTSHARE *soupacketptr,*cursoupacketptr;
	AONTSHARE *tarpacketptr,*curtarpacketptr;
	CHUNK *chunkptr,*curchunkptr,*tmpchunkptr;
	CHUNKGROUP *chunkgroupptr;
	int groupsize,chunksize;
	//int soupacketnum,tarpacketnum;
	BYTE *souptr,*curconptr,*conptr;
	...
	if(chunkgroupptr==NULL)
	{
		show_error("matrix","matrix_matrix_calculate_packet","No memory");
		return FALSE;
	}
	...
	soupacketptr=soupacketgtr->sharegroupptr;
	soupacketnum=soupacketgtr->groupsize;

	packetsize=soupacketptr->sizenum;
	chunksize=packetsize/w;
	
	cursoupacketptr=soupacketptr;
	for(i=0;i<datanum;i++)
	{
		souptr=cursoupacketptr->content;
		conptr=souptr;
		for(j=0;j<w;j++)
		{
			//调整指针
			curchunkptr->chunkcontent=conptr;
			curchunkptr->sizenum=chunksize;
			conptr=conptr+chunksize;
		}
		cursoupacketptr++;
	}
	//先申请一个chunk来记下中间结果
	tmpchunkptr=talloc(CHUNK,1);
	if(tmpchunkptr==NULL)
	{
		show_error("matrix","matrix_matrix_calculate_packet","No memory");
		return FALSE;
	}
	souptr=talloc(BYTE,chunksize);
	if(souptr==NULL)
	{
		show_error("matrix","matrix_matrix_calculate_packet","No memory");
		return FALSE;
	...
	windex=(pronum-datanum)*w;//计算偏移量（n-k）-（d-k）=n-d使我们要的编码块
	index=pronum-datanum;
	tmpchunkptr->chunkcontent=souptr;
	tmpchunkptr->sizenum=chunksize;
	mat=matrixptr->matrix;

	tarpacketptr=tarpacketgtr->sharegroupptr;
	curtarpacketptr=tarpacketptr;
	//开始计算
	for(i=index;i<codenum;i++)
	{
		conptr=curtarpacketptr->content;
		curconptr=conptr;

		for(j=0;j<w;j++)//每一个中有w行
		{
			//调整到表头
			curchunkptr=chunkptr;
			bzero(souptr,BYTE,chunksize);//清零
			for(k=0;k<cols;k++)
			{
				if(mat[windex]==1)
				{
					...
					if(resTF==FALSE)
					{
						show_error("matrix","matrix_matrix_calculate_packet","XOR cannot calculate");
						return FALSE;
					}
					curchunkptr++;
					index++;
				...
				//copy one chunk
				mcpy(curconptr,souptr,BYTE,chunksize);
